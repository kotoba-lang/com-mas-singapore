(ns mas_singapore.main
  "Kotodama WASM entrypoint for the Mas_singapore clean-room actor (L4) — Clojure port.
  ...
  facts (`mas_singapore.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mas_singapore")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mas_singapore.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mas_singapore-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mas_singapore-compat/ready)
